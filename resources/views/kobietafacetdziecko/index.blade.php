...
<div class="col-6 mt-auto" style="text-align:right;">
  <form id="search" action="{{ route('post.serach',['section' => $serachsection] ) }}" method="post">
    ...
    <h5><input type="submit" class="btn-sectiontext" value="ZOBACZ WSZYSTKO"></h5>
  </form>
  ...
  <div class="row">
    <ul class="nav nav-tabs" id="TUTUpreg_replace('/\s+/', '', $category->category)" role="tablist">
      ...
      <li class="nav-item" role="presentation">
        <button class="text-dark text-uppercase nav-link active" id="home-tab{{preg_replace('/\s+/', '', $sub->category)}}" data-bs-toggle="tab" data-bs-target="#home{{preg_replace('/\s+/', '', $sub->category)}}" type="button" role="tab" aria-controls="home" aria-selected="true">{{preg_replace('/\s+/', '', $sub->category)}}</button>
      </li>
      @else
      <li class="nav-item" role="presentation">
        <button class="text-dark text-uppercase nav-link" id="home-tab{{preg_replace('/\s+/', '', $sub->category)}}" data-bs-toggle="tab" data-bs-target="#home{{preg_replace('/\s+/', '', $sub->category)}}" type="button" role="tab" aria-controls="home" aria-selected="false">{{preg_replace('/\s+/', '', $sub->category)}}</button>
      </li>
      @endif
      @endforeach
    </ul>
