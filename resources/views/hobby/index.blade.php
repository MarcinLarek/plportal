...
@foreach($categories as $category)
  ...
  <hr class="section-hr">
  <?php $listposts = $category->getposts()->take(8); ?>
    <div class="row">
      <div class="col-6">
        <div class="row">
          @foreach($listposts->take(2) as $listpost)
      ...
      <div class="col-6">
        @foreach($listposts->skip(2)->take(2) as $listpost)
        <div class="row mb-3">
          <div class="col-5">
            <a href="{{ route('post.show', ['post' => $listpost, 'section' => $listpost->getsection()]) }}">
            <div class="section-righttableimage" style="background-image: url('/storage/{{ $listpost->image }}')">
            </div>
            </a>
          </div>
          <div class="col-7">
            <span><a class="text-section" href="{{ route('post.show', ['post' => $listpost, 'section' => $listpost->getsection()]) }}">{{$listpost->title}}</a></span>
            <p class="text-dark" style="font-weight: lighter;">{{strip_tags(substr($listpost->postcontent, 0, 100))}}</p>
          </div>
        </div>
        @endforeach
      </div>
    ...
    <div class="row mt-3">
      <div class="col-6">
        @foreach($listposts->skip(4)->take(2) as $listpost)
        <div class="row mb-3">
          <div class="col-5">
            <a href="{{ route('post.show', ['post' => $listpost, 'section' => $listpost->getsection()]) }}">
            <div class="section-righttableimage" style="background-image: url('/storage/{{ $listpost->image }}')">
      ...
      <div class="col-6">
        <div class="row">
          @foreach($listposts->skip(6)->take(2) as $listpost)
            <div class="col section-imagebox bg-section">
              <a href="{{ route('post.show', ['post' => $listpost, 'section' => $listpost->getsection()]) }}">
              <div class="col-3 w-100 section-threepostsection d-flex" style="background-image: url('/storage/{{ $listpost->image }}')">
              </div>
