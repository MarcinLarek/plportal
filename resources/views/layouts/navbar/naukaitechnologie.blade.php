  ...
  <div class="row">
    <div class="col-12 d-flex  justify-content-between headerimage w-100" style="background-image:  url('/storage/naukaitechnologienav.png');">
        <div class="text-end pb-1 w-100">
          <form id="search" action="{{route('naukaitechnologie.serach')}}" method="get">
            @csrf
...
<nav class="navbar-expand-sm bg-primary">
    <div class="collapse navbar-collapse">
        <ul class="navbar-nav mx-auto">
          ...
          <li class="nav-item">
            <a class="nav-link secondnavbaritem" href="{{ route('naukaitechnologie.index') }}">Strona Główna</a>
          </li>
          @foreach($categories as $category)
            @if($category->section == 'Nauka i Technologie')
            <li class="nav-item">
              <a class="nav-link secondnavbaritem" href="{{ route('naukaitechnologie.category', ['category' => $category->category]) }}">{{$category->category}}</a>
            </li>
            @endif
          @endforeach
        </ul>
