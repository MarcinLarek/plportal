  ...
  <div class="row">
    <div class="col-12 d-flex  justify-content-between headerimage w-100" style="background-image:  url('/storage/historianav.png');">
        <div class="text-end pb-1 w-100">
          <form id="search" action="{{route('historia.serach')}}" method="get">
            @csrf
...
<nav class="navbar-expand-sm bg-primary">
    <div class="collapse navbar-collapse">
        <ul class="navbar-nav mx-auto">
          ...
          <li class="nav-item">
            <a class="nav-link secondnavbaritem" href="{{ route('historia.index') }}">Strona Główna</a>
          </li>
          @foreach($categories as $category)
            @if($category->section == 'Historia')
            <li class="nav-item">
              <a class="nav-link secondnavbaritem" href="{{ route('historia.category', ['category' => $category->category]) }}">{{$category->category}}</a>
            </li>
            @endif
          @endforeach
        </ul>
