  ...
  <div class="row">
    <div class="col-12 d-flex  justify-content-between headerimage w-100" style="background-image:  url('/storage/spoleczenstwonav.png');">
        <div class="text-end pb-1 w-100">
          <form id="search" action="{{route('spoleczenstwo.serach')}}" method="get">
            @csrf
...
<nav class="navbar-expand-sm bg-primary">
    <div class="collapse navbar-collapse">
        <ul class="navbar-nav mx-auto">
          ...
          <li class="nav-item">
            <a class="nav-link secondnavbaritem" href="{{ route('spoleczenstwo.index') }}">Strona Główna</a>
          </li>
          @foreach($categories as $category)
            @if($category->section == 'Spoleczenstwo')
            <li class="nav-item">
              <a class="nav-link secondnavbaritem" href="{{ route('spoleczenstwo.category', ['category' => $category->category]) }}">{{$category->category}}</a>
            </li>
            @endif
          @endforeach
        </ul>
