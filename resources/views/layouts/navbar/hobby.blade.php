  ...
  <div class="row">
    <div class="col-12 d-flex  justify-content-between headerimage w-100" style="background-image:  url('/storage/hobbynav.png');">
        <div class="text-end pb-1 w-100">
          <form id="search" action="{{route('hobby.serach')}}" method="get">
            @csrf
...
<nav class="navbar-expand-sm bg-primary">
    <div class="collapse navbar-collapse">
        <ul class="navbar-nav mx-auto">
          ...
          <li class="nav-item">
            <a class="nav-link secondnavbaritem" href="{{ route('hobby.index') }}">Strona Główna</a>
          </li>
          @foreach($categories as $category)
            @if($category->section == 'Hobby')
            <li class="nav-item">
              <a class="nav-link secondnavbaritem" href="{{ route('hobby.category', ['category' => $category->category]) }}">{{$category->category}}</a>
            </li>
            @endif
          @endforeach
        </ul>
